...
CFG_PATH = r"/home/user/github/JetsonPipelineAWR2944/awr2944_stuff/test.cfg"  # <-- change this
...
def open_cli(port, baud):
    ser = serial.Serial(
        port,
        baudrate=baud,
        timeout=0.02,             # give the device time to answer
        write_timeout=1.0,
...
def send_cfg(cli_port, cfg_file, baud=CLI_BAUD):
    ...
    with open_cli(cli_port, baud) as ser:
        ...
        for pre in ("sensorStop", "flushCfg"):
            ...
            sys.stdout.write(read_until_prompt(ser, 3.0).decode(errors="ignore"))

        for i, raw in enumerate(cfg.read_text(encoding="utf-8", errors="ignore").splitlines(), 1):
